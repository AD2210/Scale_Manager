...
#[Route('/api/finish', name: 'api_finish_')]
class FinishOperationController extends AbstractController
{
    ...
    public function updateOperation(
        ...
        if ($content['field'] === 'finishProcess') {
            // Cas de la suppression
            if (empty($content['value']) && !empty($content['operationId'])) {
                $operationId = $content['operationId'];
                $operation = $em->getRepository(FinishOperation::class)->find($operationId);
            ...
            if (!empty($content['operationId'])) {
                $operation = $em->getRepository(FinishOperation::class)->find($content['operationId']);
                if (!$operation) {
