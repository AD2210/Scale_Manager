...
#[Route('/api/quality', name: 'api_quality_')]
class QualityOperationController extends AbstractController
{
    ...
    public function updateOperation(
        ...
        if ($content['field'] === 'qualityProcess') {
            // Cas de la suppression
            if (empty($content['value']) && !empty($content['operationId'])) {
                $operationId = $content['operationId'];
                $operation = $em->getRepository(QualityOperation::class)->find($operationId);
            ...
            if (!empty($content['operationId'])) {
                $operation = $em->getRepository(QualityOperation::class)->find($content['operationId']);
                if (!$operation) {
                    return $this->json(['error' => 'Opération non trouvée'], Response::HTTP_NOT_FOUND);
                }
                $operation->setQualityProcess($process);
            } else {
                // Cas de la création
                $operation = new QualityOperation();
