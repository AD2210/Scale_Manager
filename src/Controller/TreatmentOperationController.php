...
#[Route('/api/treatment', name: 'api_treatment_')]
class TreatmentOperationController extends AbstractController
{
    ...
    public function updateOperation(
        ...
        if ($content['field'] === 'treatmentProcess') {
            // Cas de la suppression
            if (empty($content['value']) && !empty($content['operationId'])) {
                $operationId = $content['operationId'];
                $operation = $em->getRepository(TreatmentOperation::class)->find($operationId);
            ...
            if (!empty($content['operationId'])) {
                $operation = $em->getRepository(TreatmentOperation::class)->find($content['operationId']);
                if (!$operation) {
